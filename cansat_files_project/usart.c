...
#define UART_REGS (&(SERCOM3_REGS->USART_INT)) 
	...
	while ((UART_REGS->SERCOM_SYNCBUSY & 0x1) != 0)
		asm("nop");
    
    /* Switch MODE[4:2] to USART with internal clock */
    ...
    /* (4) (5) (10) */
	UART_REGS->SERCOM_CTRLB = (0x0 << 6) | (0x0 << 0) | (0x0 << 13);
    /* (9) */
	UART_REGS->SERCOM_CTRLC = (0x0 << 27);
    /* (6) is implemented inherently */
    ...
    /* 
     * 8 (char) + 1 (parity) + 1 (stop) + 1 (margin) = 11 bits 
     * 12 * 3 = 36 * 1/38400 = 937.5u = 937500n
     * 11 * 3 = 33 * 1/57600 = 572916n
     */
	...
	while ((UART_REGS->SERCOM_SYNCBUSY & 0x4) != 0)
		asm("nop");
    ...
    /* Enables PB09 (RX) PMUX */
    PORT_SEC_REGS->GROUP[1].PORT_PINCFG[9] = 0x1;    
    /* Set PB09 (4*2+1) to peripheral D (SERCOM3/PAD[1]) */
    PORT_SEC_REGS->GROUP[1].PORT_PMUX[4] = 0x30;
    
    /* Enables PB08 (TX) PMUX */
